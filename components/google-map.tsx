...
declare global {
  interface Window {
    google: any
    initMap: () => void
    handlePlaceAction?: (action: string, placeId: number) => void
  }
}
...
export default function GoogleMap({ places, onPlaceSelect, onPlaceAction, selectedFilter }: GoogleMapProps) {
  ...
  useEffect(() => {
    if (!map || !window.google || !window.google.maps || !(map instanceof window.google.maps.Map)) return;
    ...
    window.handlePlaceAction = function (action: string, placeId: number) {
