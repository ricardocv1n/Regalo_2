...
export default function MessageCard({ message, onClick, onEdit, onDelete, onToggleFavorite, index }: MessageCardProps) {
  ...
  return (
    <Card
      className={`
        ...
        ${message.isFavorite ? "ring-2 ring-yellow-400 ring-opacity-50" : ""}
      `}
    ...
    >
      ...
      <div className="absolute top-4 right-4 w-8 h-8 bg-pink-600 rounded-full opacity-80 flex items-center justify-center">
        <Heart className={`h-4 w-4 ${message.isFavorite ? "text-yellow-400" : "text-white"}`} />
      </div>
      ...
      <CardContent className="p-6 relative z-10">
        ...
        >
          <Button
            variant="ghost"
            size="icon"
            className={`h-8 w-8 ${message.isFavorite ? "text-yellow-500" : "text-gray-500"} hover:text-yellow-600 hover:bg-yellow-50`}
            onClick={(e) => {
              e.stopPropagation()
              onToggleFavorite()
            }}
