...
export default function MusicPlayer({ initialSongs }: MusicPlayerProps) {
  ...
  // Inicializar el reproductor y el contexto de audio SOLO UNA VEZ
  useEffect(() => {
    ...
    const loadSavedState = () => {
      ...
      if (savedState) {
        try {
          ...
          // Resetear el estado si hay un error
  ...
  // Configurar el contexto de audio y la fuente SOLO UNA VEZ POR ELEMENTO DE AUDIO
  useEffect(() => {
    const audioEl = audioRef.current;
    if (!audioEl) return;

    // Limpia cualquier fuente/contexto anterior
  ...
  // Cuando se reproduce, reanudar el contexto si está suspendido
  useEffect(() => {
  ...
  }, [isPlaying, currentSongIndex, repeatMode, isShuffle, songs.length])
